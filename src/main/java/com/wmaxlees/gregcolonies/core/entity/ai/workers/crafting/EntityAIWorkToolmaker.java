...
public class EntityAIWorkToolmaker extends AbstractEntityAICrafting<JobToolmaker, BuildingToolmaker> {
    /**
    ...
    public EntityAIWorkToolmaker(@NotNull final JobToolmaker job) {
        ...
        super.registerTargets(
                new AITarget<IAIState>(CRAFT_TOOL, this::craftTools, TICKS_SECOND));
    ...
    private ResourceLocation getNextCraftableTool() {
        Set<ResourceLocation> workOrders = building.getFirstModuleOccurance(ToolmakerWorkordersModule.class).getWorkorders();
        Set<ResourceLocation> toolsToKeepStocked = building.getFirstModuleOccurance(ToolmakerToolsModule.class).getToolsToKeepStocked();
