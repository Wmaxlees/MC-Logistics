package com.wmaxlees.gregcolonies.core.network.messages.splitting;
...
/**
 * Represents a class that wrappers other messages in byte form and is used to split the wrapped
 * messages data into several chunks.
 */
public class SplitPacketMessage implements IMessage {
  ...
  /**
   * The id of the message inside the splitting logic. Identical to the index codec system in
   * SimpleChannel-
   */
  ...
  public void onExecute(final NetworkEvent.Context ctxIn, final boolean isLogicalServer) {
    try {
      ...
      synchronized (Network.getNetwork().getMessageCache()) {
        Network.getNetwork()
            .getMessageCache()
            .get(this.communicationId, Maps::newConcurrentMap)
            .put(this.packetIndex, this.payload);
      }
      ...
      final NetworkChannel.NetworkingMessageEntry<?> messageEntry =
          Network.getNetwork().getMessagesTypes().get(this.innerMessageId);
